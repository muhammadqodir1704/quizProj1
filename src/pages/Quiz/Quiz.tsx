...
import { fetchTestQuestions, submitTest } from "../../api/request.api"
// LaTeX rendering
...
export default function Quiz() {
  ...
  const location = useLocation() // useLocation hook'i orqali state'ga kirish
  const [selectedAnswers, setSelectedAnswers] = useState<{ [key: number]: number }>({})
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [timeLeft, setTimeLeft] = useState(30 * 60) // 30 daqiqa * 60 soniya
  ...
  const handleSubmit = async (skipConfirmation = false) => {
    if (!skipConfirmation) {
      if (!window.confirm("Testni yakunlashni xohlaysizmi? Javoblaringiz o'zgartirib bo'lmaydi!")) {
        return
      }
    }
    setIsSubmitting(true)
    const answerIds = Object.values(selectedAnswers)
    const answeredQuestionIds = Object.keys(selectedAnswers).map(Number)
    const allQuestionIds = questions?.map((q) => q.id) || []
    const unansweredQuestionIds = allQuestionIds.filter((id) => !answeredQuestionIds.includes(id))
    try {
      const result = await submitTest(token!, studentName!, answerIds, unansweredQuestionIds)
      sessionStorage.removeItem("studentName")
      // sessionStorage.removeItem("testToken") // Token ni saqlab qolamiz, chunki result sahifasida kerak
      navigate(`/result/${result.id}`, { state: { testResult: result, token: token } })
    } catch (error) {
      Modal.error({
        title: "Xatolik",
  ...
  if (questionDetails) {
    return (
      <div className={`min-h-screen p-6 flex items-center justify-center ${
        ...
        <Card className={`w-full max-w-2xl rounded-2xl shadow-xl border-0 p-6 ${
          isDarkMode ? 'bg-gray-800 text-white' : 'bg-white'
        }`}>
          <Title level={3} className={`text-center mb-6 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
            Savol #{questionDetails.question_number} Tahlili
          </Title>

          <div className={`mb-6 p-4 rounded-lg ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'} border`}>
          ...
          <div className="mb-6">
            ...
            <List
              ...
              renderItem={(item, index) => (
                <List.Item className={`py-2 ${index !== questionDetails.options.length - 1 ? 'border-b border-gray-200 dark:border-gray-600' : ''}`}>
          ...
          <div className={`mb-6 p-4 rounded-lg ${isDarkMode ? 'bg-green-900 border-green-700' : 'bg-green-50 border-green-200'} border`}>
          ...
          <div className={`mb-8 p-4 rounded-lg ${isDarkMode ? 'bg-red-900 border-red-700' : 'bg-red-50 border-red-200'} border`}>
      ...
      <Affix offsetTop={0}>
        <div className={`px-6 py-4 shadow-lg ${
          ...
          <Row justify="space-between" align="middle">
            <Col>
              <Space size="large">
                <Statistic title="Javoblangan" value={answeredCount} suffix={`/ ${questions.length}`} valueStyle={{ color: '#1890ff', fontSize: "20px" }} />
